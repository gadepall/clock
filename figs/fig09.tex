\FloatBarrier
\begin{figure*}[p]
    \centering

    % ------------------ Truth Table ------------------
    \input{tables/logic/table4.tex}

    % ------------------ K-Maps (2x2 layout) ------------------
    \vspace{0.5cm}

    % ---------- Row 1 ----------
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \begin{karnaugh-map}[4][4][1][][]
            \maxterms{1,3,5,7,9}
            \minterms{0,2,4,6,8}
            \indeterminants{10,11,12,13,14,15}

            \implicantedge{0}{8}{2}{10}
            \draw[color=black, ultra thin] (0, 4) --
                node [pos=0.7, above right, anchor=south west] {$XW$}
                node [pos=0.7, below left, anchor=north east] {$ZY$} 
                ++(135:1);
        \end{karnaugh-map}
        \subcaption{$A = W_1^{\prime}$ }
    
    
        \label{fig:kmapA-4-6}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \begin{karnaugh-map}[4][4][1][][]
            \minterms{1,2,5,6}
            \maxterms{0,3,4,7,8,9}
            \indeterminants{10,11,12,13,14,15}

            \implicant{1}{5}
            \implicant{2}{10}

            \draw[color=black, ultra thin] (0, 4) --
                node [pos=0.7, above right, anchor=south west] {$XW$}
                node [pos=0.7, below left, anchor=north east] {$ZY$} 
                ++(135:1);
        \end{karnaugh-map}
        \subcaption{$B = W_1 X_1^{\prime} Z_1^{\prime} + W_1^{\prime} X_1$}
        \label{fig:kmapB-4-6}
    \end{subfigure}

    \vspace{0.5cm}

    % ---------- Row 2 ----------
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \begin{karnaugh-map}[4][4][1][][]
            \minterms{3,4,5,6}
            \maxterms{0,1,2,7,8,9}
            \indeterminants{10,11,12,13,14,15}

            \implicant{4}{5}    
            \implicant{3}{3}
            \implicantedge{4}{4}{6}{6}   

            \draw[color=black, ultra thin] (0, 4) --
                node [pos=0.7, above right, anchor=south west] {$XW$}
                node [pos=0.7, below left, anchor=north east] {$ZY$} 
                ++(135:1);
        \end{karnaugh-map}
        \subcaption{$C = X_1^{\prime} Y_1 + W_1^{\prime} Y_1 + W_1 X_1 Y_1^{\prime}$}
        \label{fig:kmapC-4-6}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \begin{karnaugh-map}[4][4][1][][]
            \minterms{7,8}
            \maxterms{0,1,2,3,4,6,5,9}
            \indeterminants{10,11,12,13,14,15}

            \implicant{7}{15}    
            \implicantedge{12}{8}{14}{10}   

            \draw[color=black, ultra thin] (0, 4) --
                node [pos=0.7, above right, anchor=south west] {$XW$}
                node [pos=0.7, below left, anchor=north east] {$ZY$} 
                ++(135:1);
        \end{karnaugh-map}
        \subcaption{$D = W_1^{\prime} Z_1 + W_1 X_1 Y_1 $}
        \label{fig:kmapD-4-6}
    \end{subfigure}

    \caption{Counting 0-9}
    \label{fig:0-9}
\end{figure*}

